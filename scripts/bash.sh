...
bash_api() {
    ...
    cd /home/webhookd/runner/"$dir_name/bash"
    # set start time
    start_time="$(date +%s%N)"
    # run script
    source ../bashrun
...
bash_results() {
    ...
    stdout="$(cat /home/webhookd/runner/"$dir_name"/bashstdout | sed -r 's/\x1B(\[[0-9;]*[JKmsu]|\(B)//g')"
    # get stderr from file and remove colors
    stderr="$(cat /home/webhookd/runner/"$dir_name"/bashstderr | sed -r 's/\x1B(\[[0-9;]*[JKmsu]|\(B)//g')"
    # get log number
    ...
    jq -n --arg pg "$raw" --arg so "$stdout" --arg se "$stderr" --arg ex "$exitcode" --arg tm "$time" --arg ln "$log_num" '{"program":$pg,"stdout":$so,"stderr":$se,"exit":$ex,"time":$tm,"logNum":$ln}'
...
mkdir -p /home/webhookd/runner/"$dir_name/bash"
